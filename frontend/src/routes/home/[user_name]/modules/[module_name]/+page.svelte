<script>
  ...
  async function loadQuizzes() {
    ...
    //Load quizzes
    const quizQuery = await fetch(`http://localhost:8000/users/me/modules/${moduleName}/quizzes/` , {
      method : 'GET',
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json"
  ...
  async function registerQuiz(event) {
    ...
    const res = await fetch(`http://localhost:8000/users/me/modules/${moduleName}/quizzes/`, {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json"
