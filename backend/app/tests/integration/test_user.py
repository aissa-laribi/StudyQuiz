...
@pytest.mark.anyio
async def test_resend_verification_email(async_app_client):
    #1.create user
    data = {
        "email": "testsstudyquiz@gmail.com",
        "password": "StrongPwd1234,,,,tewfw4g",
    ...
    assert f"/confirm-email?token=" in response.json()
    assert len(response.json().split('\n')) == 4
    old_token = str(response.json().split('\n')[2].split('=')[1])
    ...
    response = await async_app_client.post("/users/token", data=form_data, headers={"Content-Type": "application/x-www-form-urlencoded"})
    assert response.status_code == 200
    ...
    #4.Request confirmation email
    resend_response = await async_app_client.post("/users/me/resend-verificationtoken?prod=false",
        headers={"Authorization": f"Bearer {access_token}"}
    )
    assert resend_response.status_code == 200
    assert "/confirm-email?token=" in resend_response.json()

    #5.Check old token != new token, and user still not verified
    new_token = str(resend_response.json().split('\n')[2].split('=')[1])
